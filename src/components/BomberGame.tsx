...
const getBomberDirectionDelta = (direction: BomberDirection) => {
  switch (direction) {
    case 'up':
      return { x: 0, y: -1 };
    case 'down':
...
const isBomberClientCellBlocked = (bomberState: any, x: number, y: number) => {
  if (!bomberState?.grid) return false;
  if (y < 0 || y >= bomberState.grid.length) return true;
  if (x < 0 || x >= (bomberState.grid[y]?.length || 0)) return true;
  const cell = bomberState.grid[y]?.[x];
  if (cell === 'solid' || cell === 'breakable') return true;
  return (bomberState.bombs || []).some((bomb: any) => bomb.x === x && bomb.y === y);
};
...
const getBomberVisualMoveMs = (moveSpeedLevel = 0) => {
  const repeatMs = getBomberMoveRepeatMs(moveSpeedLevel);
  return repeatMs * 1.08;
};
...
export default function BomberGame({ roomId, me, players, bomberState, onMove, onPlaceBomb, onDetonateRemote, canUseRemote = false }: BomberGameProps) {
  ...
  const bomberMotionRef = useRef({
    players,
    bomberState,
    meId: me?.id,
    width: bomberState?.width || 21,
    height: bomberState?.height || 15,
  });
  const [facingByPlayer, setFacingByPlayer] = useState<Record<string, 'up' | 'down' | 'left' | 'right'>>({});
  const [displayBomberPositions, setDisplayBomberPositions] = useState<Record<string, { x: number; y: number }>>({});
  ...
  const startMoveHold = (direction: BomberDirection) => {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      last = now;
      const motion = bomberMotionRef.current;
      ...
      setDisplayBomberPositions((current) => {
        ...
        Object.entries(motion.players || {}).forEach(([id, player]: [string, any]) => {
          if (typeof player?.bomberX !== 'number' || typeof player?.bomberY !== 'number') return;
          const currentPos = current[id] || { x: player.bomberX, y: player.bomberY };
          const visualMoveMs = getBomberVisualMoveMs(player.moveSpeedLevel || 0);
          const targetX = player.bomberX;
          const targetY = player.bomberY;

          const distance = Math.hypot(targetX - currentPos.x, targetY - currentPos.y);
          const step = (1000 / Math.max(110, visualMoveMs)) * dt;
          const ratio = distance > 0 ? Math.min(1, step / distance) : 1;
          const shouldSnap = !player.alive || distance > 3.5;
          const nextX = shouldSnap ? player.bomberX : currentPos.x + (targetX - currentPos.x) * ratio;
          const nextY = shouldSnap ? player.bomberY : currentPos.y + (targetY - currentPos.y) * ratio;

          if (Math.abs(nextX - currentPos.x) > 0.003 || Math.abs(nextY - currentPos.y) > 0.003) {
            changed = true;
          }

          next[id] = {
            x: Math.abs(targetX - nextX) < 0.01 ? targetX : nextX,
            y: Math.abs(targetY - nextY) < 0.01 ? targetY : nextY,
          };
        });
  ...
  const translate = (() => {
    ...
    const meDisplayPos = displayBomberPositions[me?.id] || { x: me?.bomberX || 0, y: me?.bomberY || 0 };
    const meXRatio = (meDisplayPos.x + 0.5) / width;
    const meYRatio = (meDisplayPos.y + 0.5) / height;
          ...
          >
            ...
            >
              ...
              {alivePlayers.map((player: any) => {
                const displayPos = displayBomberPositions[player.id] || { x: player.bomberX, y: player.bomberY };
                ...
                <div
                  ...
                  style={{
                    ...
                    left: `calc(${displayPos.x * cellWidthPercent}% + 3px)`,
                    top: `calc(${displayPos.y * cellHeightPercent}% - 5px)`,
